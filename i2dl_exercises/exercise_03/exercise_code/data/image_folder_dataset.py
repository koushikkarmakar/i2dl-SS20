...
class ImageFolderDataset(Dataset):
    ...
    def make_dataset(directory, class_to_idx):
        ...
        for label in class_to_idx:
            currentDict = os.path.join(directory,label)
            files = os.listdir(currentDict)
            samples_per_class = len(files)
            for i in range(samples_per_class):
                image_path = os.path.join(currentDict,
            str(i+1).zfill(4) + ".png")
    ...
    def __getitem__(self, index):
        ...
        if(self.transform is None):
            data_dict = {"image": np.asarray(Image.open(self.images[index])), "label": self.labels[index]}
        else:
            data_dict = {"image": np.asarray(self.transform(Image.open(self.images[index]))), "label": self.labels[index]}
